...
async function confirmCombination(req, res, next) {
  try {
    ...
    const bothOperatorsConfirmed = request.recipientConfirmed;
    const workflowStatus = bothOperatorsConfirmed ? 'confirmed' : 'awaiting_recipient_confirmation';
    const status = bothOperatorsConfirmed ? 'assigned' : request.status;
    await request.update({ transporterConfirmed: true, workflowStatus, status });
